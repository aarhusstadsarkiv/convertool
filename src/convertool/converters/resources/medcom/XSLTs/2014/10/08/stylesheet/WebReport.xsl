...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
                  ...
                  <tbody>
                    ...
                    <tr>
                      <tr>
                        ...
                        <td>
                          <xsl:value-of select="m:Sender/m:DepartmentName"/>, <xsl:value-of
                            select="m:Sender/m:UnitName"/>, <xsl:value-of
                            select="m:Sender/m:OrganisationName"/>
                        </td>
                      </tr>
                      <tr>
                        <td align="right">
                        ...
                        <td>
                          <xsl:value-of
                            select="m:LaboratoryResults/m:GeneralResultInformation/m:LaboratoryInternalProductionIdentifier"
                          />
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <xsl:for-each
                            select="m:RequisitionInformation/m:Sample/m:SamplingDateTime">
                        ...
                        <tr>
                          ...
                          <td>
                            <xsl:for-each select="m:LaboratoryResults/m:Comments">
                              <xsl:call-template name="codeTypeTable"/>
                            </xsl:for-each>
                          </td>
                        </tr>
                      </tr>
        ...
        <xsl:for-each select="m:Investigation">
          <xsl:element name="li">
            ...
            <xsl:element name="a">
              ...
              </xsl:if> Prøve <xsl:value-of select="position()"/>
            </xsl:element>
          </xsl:element>
        </xsl:for-each>
      </ul>
      ...
      <xsl:for-each select="m:Investigation">
        <xsl:element name="div">
          ...
          <xsl:for-each select="m:Examination">
            <xsl:call-template name="examnimationTable"/>
          </xsl:for-each>


          <xsl:for-each select="m:CultureFindings">
            <xsl:call-template name="microscopicExaminationTable"/>
          </xsl:for-each>


          <xsl:for-each select="m:CultureFindings">
            <xsl:call-template name="cultureFindingsTable14"/>
          </xsl:for-each>

          <xsl:apply-templates select="m:MicroscopicFindings"/>

          <xsl:for-each select="m:QuantitativeFindings">
            <xsl:call-template name="quantativeFindingsTable"/>
          </xsl:for-each>
        </xsl:element>
      </xsl:for-each>
    </div>
  ...
  <xsl:template name="examnimationTable">
    <table>
      <br/>
      <tr>
        <td>
  ...
  <xsl:template name="analysisFindingsTable">
    <table width="100%">
      <xsl:attribute name="bordercolor">
        <xsl:value-of select="'black'"/>
      </xsl:attribute>
        ...
        <tr>
          ...
          <td>          
            <xsl:for-each select="m:Analysis/m:Text/m:Paragraph">           
              <xsl:call-template name="paragraphListing"/>
            </xsl:for-each>   
          </td>         
          <td> 
  ...
  <xsl:template name="codeTypeTable">
    <table>
      <xsl:attribute name="bordercolor">
        <xsl:value-of select="'black'"/>
      </xsl:attribute>
  ...
  <xsl:template name="codeTypeListing">
  ...
  <xsl:template name="paragraphListing">
    <table>
      <tr>
        <td width="100%" >
          <xsl:value-of select="."/> 
  ...
  <!--</script>  -->
  <xsl:template name="quantativeFindingsTable">
    <table>
      <thead>
        <th align="left">Måleresultater</th>

      </thead>
      <tr>
        <td>
          <h4>Tolkning af anayseresultater</h4>
          <xsl:for-each select="m:InterPretation">
            <xsl:call-template name="codeTypeTable"/>
          </xsl:for-each>
        </td>
      </tr>
      <!--  @RUN gl afsnit 2020.03.23
      <xsl:if test="count(m:AnalysisFindings) > 0">
        <tr>
          <td>
            <h4>Analyse navn og id</h4>
            <xsl:for-each select="m:AnalysisFindings/m:Analysis">
              <xsl:call-template name="codeTypeTable"/>
            </xsl:for-each>
          </td>
        </tr>
      ...
      <xsl:if test="count(m:AnalysisFindings) > 0">
        <tr>
          <td>
            <h4>Analyse og Fortolkning
            </h4>
            <xsl:for-each select="m:AnalysisFindings">
              <xsl:call-template name="analysisFindingsTable"/>
            </xsl:for-each>
          </td>
        </tr>
      </xsl:if>
      ...
      <xsl:if test="count(m:Comments) > 0">
        <tr>
          <td>
            <h4>Kommentar til analyseresultatet</h4>
  ...
  <xsl:template match="m:MicroscopicFindings">
    ...
    <xsl:call-template name="microscopicFindingsTable"/>

    <h5>Grouping</h5>
    <xsl:for-each select="m:Grouping">
      <xsl:call-template name="microscopicCodesTable"/>
    </xsl:for-each>
  ...
  <xsl:template name="cultureFindingsTable14">
   ...
   <xsl:template name="microscopicExaminationTable">
  ...
  <xsl:template name="microscopicFindingsTable">
...
<!-- Tabel til visning af koder for mikroskopiske fund -->
  <xsl:template name="microscopicCodesTable">
